...
class Node:
    ...
    def __init__(self, 
        ...
        self.snapshot : tuple = game.get_snapshot()
        self.decision_player : int = game.decision_player # decision player used when get_possible_action is called on this node
    ...
    def add_child(self, action : ActionType, game : Armada, policy : float = 0, value : float = 0, action_index : int = 0) -> Node :
        ...
        """
        node = Node(game=game, action=action, policy=policy, value=value, action_index=action_index)
    ...
    def select_child(self, use_policy : bool) -> Node:
        ...
        for child in self.children:
            if use_policy: ucb = self._get_pucb(child)
            else : ucb = self._get_ucb(child)
    ...
    def _get_ucb(self, child : Node) -> float:
        if child.visits == 0:
            return float('inf')
        q_value : float = child.wins / child.visits
        return q_value + Config.EXPLORATION_CONSTANT * math.sqrt(math.log(self.visits)) / child.visits

    def _get_pucb(self, child : Node) -> float:
        if child.visits == 0:
            q_value : float = 0
        else: 
            q_value : float = child.wins / child.visits
...
class MCTS:
    ...
    def para_search(self, sim_players : dict[int, int], *,deep_search:bool = False) -> dict[int, np.ndarray]:
        ...
        for para_index in para_indices:
            ...
            root_node : Node = self.player_roots[para_index][sim_player]
        ...
        for _ in range(mcts_iteration):
            ...
            for para_index in para_indices:
                ...
                if (winner := game.winner) is not None or node.children:
                    value = winner if winner is not None else node.value
                    self._backpropagation(path, value)
            ...
            if expandable_node_indices :
                ...
                for output_index, para_index in enumerate(expandable_node_indices):
                    ...
                    if len(path) == 1:
                        policy = (1-Config.DIRICHLET_EPSILON) * policy + Config.DIRICHLET_EPSILON * np.random.dirichlet(np.full(len(policy), Config.DIRICHLET_ALPHA))
                    ...
                    policy = self._mask_policy(policy, valid_actions, action_map)

                    self._expansion(node, para_index, valid_actions, action_map, value, policy)
    ...
    def get_random_best_action(self, para_index : int, decision_player : int, game_round : int) -> ActionType:
        ...
        temperature = Config.TEMPERATURE / game_round
    ...
    def advance_tree(self, para_index : int, action: ActionType, snapshot : tuple) -> None:
        """
        ...
        for player in (-1, 1):
            ...
            matching_child = next((child for child in self.player_roots[para_index][player].children if child.action == action), 
                                  # on chance node, there may not be matching child. add dynamically
                                  self.player_roots[para_index][player].add_child(action, self.para_games[para_index]))
    ...
    def _selection(self, root_node: Node, para_index:int) -> deque[Node]:
        """
        Select down the tree from a root node to a leaf node.
        """
        node : Node = root_node
        path : deque[Node] = deque([node])
    ...
    def _get_value_policy(self, encoded_states: list[dict[str, np.ndarray]], phases: list[Phase]) -> tuple[np.ndarray, np.ndarray]:
        """
    ...
    def _mask_policy(self, policy:np.ndarray, valid_actions: list[ActionType], action_map: dict[ActionType, int]) -> np.ndarray:
    ...
    def _expansion(self, node:Node, para_index:int, valid_actions: list[ActionType], action_map: dict[ActionType, int], value : float, policy : np.ndarray) -> None:
        for action in valid_actions:
            action_index = action_map[action]
    ...
    def _backpropagation(self, path: deque[Node], value: float) -> None:
        ...
        while path:
            ...
            if path :
                perspective_player = path[-1].decision_player
    ...
    def _get_final_action_probs(self, para_index:int, sim_player:int, max_size:int) -> np.ndarray:
