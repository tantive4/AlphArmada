...
class MCTS:
    ...
    def search_parallel(self, iterations: int, num_processes: int = 4) -> None:
            """
            Runs the MCTS search in parallel using multiple processes.
            """
            if len(self.game.get_possible_actions()) == 1:
                # Handle single-decision optimization as before
                action = self.game.get_possible_actions()[0]
                self.game.apply_action(action)
                self.root.add_child(action, self.game)
                return

            with multiprocessing.Pool(processes=num_processes) as pool:
                for i in range(iterations // num_processes):
                    # 1. Selection and Expansion for each worker
                    nodes_to_simulate = []
                    game_states = []
                    ...
                    # 2. Simulation in Parallel
                    results = pool.map(self._run_simulation, game_states)

                    # 3. Backpropagation
                    for node, simulation_result in zip(nodes_to_simulate, results):
                        temp_node = node
                        while temp_node is not None:
                            perspective_player = temp_node.parent.decision_player if temp_node.parent else self.root.decision_player
    ...
    def _run_simulation(self, game: Armada) -> float:
        """
        A helper method to run a single simulation. This will be called by the worker processes.
        """
        return game.play(max_simulation_step=1000)
