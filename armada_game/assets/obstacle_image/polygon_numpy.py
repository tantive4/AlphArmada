...
def _load_cv2():
    try:
        ...
        raise ImportError("Install opencv-python to use polygon extraction helpers.") from exc
    return cv2
...
def save_polygon_visualization(coordinates: np.ndarray, output_filename: str):
    ...
    # 4. Save the image
    cv2.imwrite(str(output_filename), vis_image)
...
if __name__ == "__main__":
    ...
    try:
        ...
        coords = get_polygon_from_png(ASSET_DIR / "Asteroid3.png")
        print(f"Loaded {coords.shape[0]} coordinates.")
        print(coords)
        # 2. Save the visualization
        save_polygon_visualization(coords, ASSET_DIR / "polygon_check.png")
